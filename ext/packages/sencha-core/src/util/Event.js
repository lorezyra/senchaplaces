  ...
  return {
    ...
    getFireInfo: function(listener, fromWrapped) {
        ...
        if (listener.lateBound) {
            ...
            // For handlers declared as function references we skip resolveListenerScope
            // and simply use the observable instance as the default scope.
            // Named scopes other than controller are fine ('self' and 'this' just resolve
            // to the instance).  Controller scope with a function reference handler is invalid.
            if (namedScope && namedScope.isController) {
                // This is not a debug-mode only error because removing it will allow the
                // handler to be called with the string 'controller' as the scope
                Ext.Error.raise("Cannot resolve scope:'controller' for '" + this.name +
                    "' listener on " + observable.$className + " instance. Controller " +
                    "scope is invalid when the handler is a function reference.");
